...
fracs = []
vectors = Exp.param_s_vecs.transpose()

for vec in vectors:
    f0 = np.linalg.norm(vec[0:n])**2
    f1 = np.linalg.norm(vec[n:2*n])**2
    f2 = np.linalg.norm(vec[2*n:3*n])**2
    f3 = np.linalg.norm(vec[3*n:])**2
    fracs.append([f0,f1,f2,f3])
